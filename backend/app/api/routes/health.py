...
async def add_condition(payload: HealthConditionCreate, _admin: dict = Depends(get_current_admin)):
    ...
    result = supabase.table("health_condition").insert(payload.model_dump()).execute()
    if not result.data:
...
async def get_conditions():
    ...
    supabase = get_supabase_admin()
    result = supabase.table("health_condition").select("*").execute()
    return result.data
...
async def get_condition(condition_id: UUID, _user: dict = Depends(get_current_user)):
    supabase = get_supabase_user()
    result = supabase.table("health_condition").select("*").eq("id", str(condition_id)).maybe_single().execute()
    if not result.data:
...
async def update_condition(condition_id: UUID, payload: HealthConditionUpdate, _admin: dict = Depends(get_current_admin)):
    ...
    result = supabase.table("health_condition").update(update_data).eq("id", str(condition_id)).execute()
    if not result.data:
        raise HTTPException(status_code=404, detail="Condition not found")
    return result.data[0]
...
async def delete_condition(condition_id: UUID, _admin: dict = Depends(get_current_admin)):
    supabase = get_supabase_admin()
    supabase.table("health_condition").delete().eq("id", str(condition_id)).execute()
    return None