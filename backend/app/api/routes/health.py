...
async def add_condition(payload: HealthConditionCreate, _user: dict = Depends(get_current_user)):
...
async def get_conditions(_user: dict = Depends(get_current_user)):
    supabase = get_supabase_admin()
...
async def update_condition(condition_id: UUID, payload: HealthConditionCreate, _user: dict = Depends(get_current_user)):
    supabase = get_supabase_admin()
...
async def delete_condition(condition_id: UUID, _user: dict = Depends(get_current_user)):
    supabase = get_supabase_admin()
